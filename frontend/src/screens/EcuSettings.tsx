...
export default function EcuSettings() {
  const [settings, setSettings] = useState<Record<string, number>>(
    ecuSettings.reduce((acc, s) => ({ ...acc, [s.key]: s.value }), {} as Record<string, number>)
  );
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-start gap-3 rounded-lg border border-accent-yellow/30 bg-accent-yellow/10 p-4">
            ...
            <p className="text-sm text-foreground-secondary">
              <span className="font-medium text-foreground">Expert mode</span> &middot; ECU settings affect engine behavior. Confirm with your technician before changing values on track.
            </p>
          </div>
